...
static char *cf_string(char *val) { return xstrdup(val); }

static int cf_int(char *val) {
  char *end;
  errno = 0;
  long int x = strtol(val, &end, 10);
  if (errno || end == val || end && *end) cf_err("Invalid number");
  if ((long int)(int)x != x) cf_err("Number out of range");
  return x;
}
...
static void cf_entry_compound(char *key, char *subkey, char *val) {
  if (strncmp(key, "box", 3)) cf_err("Unknown configuration section");
...
static void cf_check(void) {
  if (!cf_box_root || !cf_cg_root || !cf_first_uid || !cf_first_gid ||
      !cf_num_boxes)
...
void cf_parse(void) {
  ...
  if (!f) die("Cannot open %s: %m", CONFIG_FILE);

  char line[MAX_LINE_LEN];
  while (fgets(line, sizeof(line), f)) {
    line_number++;
    char *nl = strchr(line, '\n');
    if (!nl) cf_err("Line not terminated or too long");
    *nl = 0;
    ...
    while (*s && *s != ' ' && *s != '\t' && *s != '=') s++;
    while (*s == ' ' || *s == '\t') *s++ = 0;
    if (*s != '=') cf_err("Syntax error, expecting key=value");
    *s++ = 0;
    while (*s == ' ' || *s == '\t') *s++ = 0;
