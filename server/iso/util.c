...
void make_dir(char *path) {
  ...
  struct stat st;
  if (stat(path, &st) < 0) die("Cannot stat %s: %m", path);
...
static int rmtree_helper(const char *fpath, const struct stat *sb,
  ...
  if (S_ISDIR(sb->st_mode)) {
    if (rmdir(fpath) < 0) die("Cannot rmdir %s: %m", fpath);
  } else {
    if (unlink(fpath) < 0) die("Cannot unlink %s: %m", fpath);
  }
  return 0;
}
...
void close_all_fds(void) {
  ...
  if (!dir) die("Cannot open /proc/self/fd: %m");
  ...
  while (e = readdir(dir)) {
    ...
    if (*end) continue;
    if (fd >= 0 && fd <= 2 || fd == dir_fd || fd == fd_to_keep) continue;
    close(fd);
  }
...
void meta_open(const char *name) {
  ...
  if (setfsuid(getuid()) < 0) die("Failed to switch FS UID: %m");
  metafile = fopen(name, "w");
  if (setfsuid(geteuid()) < 0) die("Failed to switch FS UID back: %m");
  if (!metafile) die("Failed to open metafile '%s'", name);
  fd_to_keep = fileno(metafile);
}

void meta_close(void) {
  if (metafile && metafile != stdout) fclose(metafile);
}
