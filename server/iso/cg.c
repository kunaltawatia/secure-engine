...
static int cg_read(cg_controller controller, const char *attr, char *buf) {
  ...
  if (n < 0) {
    if (maybe) goto fail_close;
    die("Cannot read %s: %m", path);
  }
  if (n >= CG_BUFSIZE - 1) die("Attribute %s too long", path);
  if (n > 0 && buf[n - 1] == '\n') n--;
  buf[n] = 0;

  if (verbose > 1) msg("CG: Read %s = <%s>\n", attr, buf);
...
cg_write(cg_controller controller, const char *attr, const char *fmt, ...) {
  ...
  if (verbose > 1) msg("CG: Write %s = %s", attr, buf);
...
void cg_prepare(void) {
  ...
  FOREACH_CG_CONTROLLER(controller) {
    ...
    if (stat(path, &st) >= 0 || errno != ENOENT) {
      ...
      if (rmdir(path) < 0) die("Failed to reset control group %s: %m", path);
    }
...
void cg_stats(void) {
  ...
  if (cg_read(CG_MEMORY, "?memory.max_usage_in_bytes", buf)) mem = atoll(buf);
  if (cg_read(CG_MEMORY, "?memory.memsw.max_usage_in_bytes", buf)) {
    memsw = atoll(buf);
    if (memsw > mem) mem = memsw;
  }
  if (mem) meta_printf("cg-mem:%lld\n", mem >> 10);
...
void cg_remove(void) {
  ...
  FOREACH_CG_CONTROLLER(controller) {
    ...
    // first --init)
    if (!cg_read(controller, "?tasks", buf)) continue;
    ...
    if (rmdir(path) < 0) die("Cannot remove control group %s: %m", path);
  }
}
