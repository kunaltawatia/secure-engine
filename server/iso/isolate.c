...
static void NONRET box_exit(int rc) {
  ...
  if (rc < 2 && cleanup_ownership) chowntree("box", orig_uid, orig_gid);
...
void __attribute__((format(printf, 1, 2))) msg(char *msg, ...) {
  ...
  if (verbose) {
    int len = strlen(msg);
    if (len > 0) partial_line = (msg[len - 1] != '\n');
...
static int read_proc_file(char *buf, char *name, int *fdp) {
  ...
  if (*fdp < 0) {
    ...
    *fdp = open(buf, O_RDONLY);
    if (*fdp < 0) return 0;  // This is OK, the process could have finished
  }
  ...
  if (c >= PROC_BUF_SIZE - 1) die("/proc/$pid/%s too long", name);
  buf[c] = 0;
  return 1;
}
...
static int get_run_time_ms(struct rusage *rus) {
  if (cg_enable && cg_timing) return cg_get_run_time_ms();
  ...
  if (!read_proc_file(buf, "stat", &proc_stat_fd)) return 0;
  x = buf;
  while (*x && *x != ' ') x++;
  while (*x == ' ') x++;
  if (*x++ != '(') die("proc stat syntax error 1");
  while (*x && (*x != ')' || x[1] != ' ')) x++;
  while (*x == ')' || *x == ' ') x++;
...
static void check_timeout(void) {
  if (wall_timeout) {
    ...
    if (wall_ms > wall_timeout) err("TO: Time limit exceeded (wall clock)");
    if (verbose > 1) fprintf(stderr, "[wall time check: %d msec]\n", wall_ms);
  }
  if (timeout) {
    int ms = get_run_time_ms(NULL);
    if (verbose > 1) fprintf(stderr, "[time check: %d msec]\n", ms);
    if (ms > timeout && ms > extra_timeout) err("TO: Time limit exceeded");
  }
}
...
static void box_keeper(void) {
  ...
  if (ticks_per_sec <= 0) die("Invalid ticks_per_sec!");
  ...
  for (;;) {
    ...
    if (p != proxy_pid) die("wait4: unknown pid %d exited!", p);
    proxy_pid = 0;
    ...
    if (n != sizeof(stat)) die("Did not receive exit status from proxy");

    // At this point, the rusage includes time spent by the proxy's children.
    final_stats(&rus);
    if (timeout && total_ms > timeout) err("TO: Time limit exceeded");
...
static void setup_root(void) {
  if (mkdir("root", 0750) < 0 && errno != EEXIST) die("mkdir('root'): %m");
...
static void setup_credentials(void) {
  if (setresgid(box_gid, box_gid, box_gid) < 0) die("setresgid: %m");
  if (setgroups(0, NULL) < 0) die("setgroups: %m");
  if (setresuid(box_uid, box_uid, box_uid) < 0) die("setresuid: %m");
  setpgrp();
  if (tty_hack && isatty(1)) {
...
static void setup_fds(void) {
  if (redir_stdin) {
    close(0);
    if (open(redir_stdin, O_RDONLY) != 0) die("open(\"%s\"): %m", redir_stdin);
  }
  if (redir_stdout) {
    close(1);
  ...
  if (redir_stderr_to_stdout) {
    if (dup2(1, 2) < 0) die("Cannot dup stdout to stderr: %m");
  }
}
...
#define RLIM(res, val) setup_rlim("RLIMIT_" #res, RLIMIT_##res, val)
  ...
  if (memory_limit) RLIM(AS, (rlim_t)memory_limit * 1024);

  if (fsize_limit) RLIM(FSIZE, (rlim_t)fsize_limit * 1024);
...
static int box_inside(char **args) {
  ...
  if (set_cwd && chdir(set_cwd)) die("chdir: %m");
...
static void setup_orig_credentials(void) {
  if (setresgid(orig_gid, orig_gid, orig_gid) < 0) die("setresgid: %m");
  if (setgroups(0, NULL) < 0) die("setgroups: %m");
  if (setresuid(orig_uid, orig_uid, orig_uid) < 0) die("setresuid: %m");
}
...
static int box_proxy(void *arg) {
  ...
  if (p < 0) die("Proxy waitpid() failed: %m");
...
static void box_init(void) {
  ...
  make_dir(box_dir);
  if (chdir(box_dir) < 0) die("chdir(%s): %m", box_dir);
}
...
static void init(void) {
  ...
  if (chown("box", orig_uid, orig_gid) < 0) die("Cannot chown box: %m");
...
static void run(char **argv) {
  ...
  if (proxy_pid < 0) die("Cannot run proxy, clone failed: %m");
  if (!proxy_pid) die("Cannot run proxy, clone returned 0");
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, short_opts, long_opts, NULL)) >= 0)
    switch (c) {
      ...
      case 'q':
        ...
        sep = strchr(optarg, ',');
        if (!sep) usage("Invalid quota specified: %s\n", optarg);
        *sep = 0;
  ...
  if (!mode) usage("Please specify an isolate command (e.g. --init, --run).\n");
  if (mode == OPT_VERSION) {
    show_version();
    return 0;
  }
  ...
  if (geteuid()) die("Must be started as root");
  if (getegid() && setegid(0) < 0) die("Cannot switch to root group: %m");
  orig_uid = getuid();
  orig_gid = getgid();
  ...
  switch (mode) {
    case OPT_INIT:
      if (optind < argc) usage("--init mode takes no parameters\n");
      init();
      break;
    case OPT_RUN:
      if (optind >= argc) usage("--run mode requires a command to run\n");
      run(argv + optind);
      break;
    case OPT_CLEANUP:
      if (optind < argc) usage("--cleanup mode takes no parameters\n");
      cleanup();
      break;
    default:
