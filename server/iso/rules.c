...
static int match_env_var(char *env_entry, struct env_rule *r) {
  if (strncmp(env_entry, r->var, r->var_len)) return 0;
...
static void apply_env_rule(char **env, int *env_sizep, struct env_rule *r) {
  ...
  int pos = 0;
  while (pos < *env_sizep && !match_env_var(env[pos], r)) pos++;
  if (pos < *env_sizep) {
    (*env_sizep)--;
  ...
  if (r->val) {
    ...
    pos = 0;
    while (environ[pos] && !match_env_var(environ[pos], r)) pos++;
    if (!(new = environ[pos])) return;
  }
...
char **setup_environment(void) {
  ...
  int orig_size = 0;
  while (orig_env[orig_size]) orig_size++;
...
struct dir_rule {
  ...
  char *outside;  // This can be an absolute path or a relative path starting
                  // with "./"
...
static const char *const dir_flag_names[] = {"rw",  "noexec", "fs",   "maybe",
                                             "dev", "tmp",    "norec"};
...
static char *sanitize_dir_path(char *path) {
  ...
  while (*p) {
    ...
    int len = next - p;
    if (len == 2 && !memcmp(p, "..", 2)) return NULL;
...
static unsigned int parse_dir_option(char *opt) {
  for (unsigned int i = 0; i < ARRAY_SIZE(dir_flag_names); i++)
    if (!strcmp(opt, dir_flag_names[i])) return 1U << i;
...
static int set_dir_action_ext(char *arg, unsigned int ext_flags) {
  ...
  if ((flags & DIR_FLAG_FS) && (flags & DIR_FLAG_TMP)) return 0;

  if (flags & DIR_FLAG_FS) {
    if (!eq || strchr(eq, '/')) return 0;
    ...
    for (char *p = out + strlen("./tmp/"); *p; p++)
      if (*p == '/') *p = ':';  // This is safe, there were no colons in "out"
    return add_dir_rule(arg, xstrdup(out), flags | DIR_FLAG_RW);
  } else if (eq) {
    if (!eq[0]) return add_dir_rule(arg, NULL, flags);
    if (eq[0] != '/' && strncmp(eq, "./", 2)) return 0;
    return add_dir_rule(arg, eq, flags);
  } else {
...
int set_dir_action(char *arg) { return set_dir_action_ext(arg, 0); }
...
void apply_dir_rules(int with_defaults) {
  ...
  for (struct dir_rule *r = first_dir_rule; r; r = r->next) {
    if (!with_defaults && (r->flags & DIR_FLAG_DEFAULT)) continue;
  ...
  for (struct dir_rule *r = first_dir_rule; r; r = r->next) {
    if (r->flags & DIR_FLAG_DISABLED) continue;
    if (!with_defaults && (r->flags & DIR_FLAG_DEFAULT)) continue;
    ...
    if (r->flags & DIR_FLAG_TMP) {
      make_dir(out);
      if (chown(out, box_uid, box_gid) < 0) die("Cannot chown %s: %m", out);
      if (chmod(out, 0700) < 0) die("Cannot chmod %s: %m", out);
    }

    unsigned long mount_flags = 0;
    if (!(r->flags & DIR_FLAG_RW)) mount_flags |= MS_RDONLY;
    if (r->flags & DIR_FLAG_NOEXEC) mount_flags |= MS_NOEXEC;
    if (!(r->flags & DIR_FLAG_DEV)) mount_flags |= MS_NODEV;
    ...
    if (r->flags & DIR_FLAG_FS) {
      ...
      if (!strcmp(in, "proc")) {
        ...
        if (mount("none", root_in, out, MS_REMOUNT | mount_flags, "hidepid=2") <
            0)
          die("Cannot re-mount proc with hidepid option: %m");
      }
    } else {
      mount_flags |= MS_BIND | MS_NOSUID;
      if (!(r->flags & DIR_FLAG_NOREC)) mount_flags |= MS_REC;
...
static int path_begins_with(char *path, char *with) {
  while (*with)
    if (*path++ != *with++) return 0;
  return (!*with || *with == '/');
}

static char *find_device(char *path) {
  FILE *f = setmntent("/proc/mounts", "r");
  if (!f) die("Cannot open /proc/mounts: %m");

  struct mntent *me;
  int best_len = 0;
  char *best_dev = NULL;
  while (me = getmntent(f)) {
    if (!path_begins_with(me->mnt_fsname, "/dev")) continue;
...
void set_quota(void) {
  if (!block_quota) return;

  char cwd[PATH_MAX];
  if (!getcwd(cwd, sizeof(cwd))) die("getcwd: %m");

  char *dev = find_device(cwd);
  if (!dev) die("Cannot identify filesystem which contains %s", cwd);
  msg("Quota: Mapped path %s to a filesystem on %s\n", cwd, dev);

  // Sanity check
  struct stat dev_st, cwd_st;
  if (stat(dev, &dev_st) < 0) die("Cannot identify block device %s: %m", dev);
  if (!S_ISBLK(dev_st.st_mode)) die("Expected that %s is a block device", dev);
  if (stat(".", &cwd_st) < 0) die("Cannot stat cwd: %m");
  if (cwd_st.st_dev != dev_st.st_rdev)
    die("Identified %s as a filesystem on %s, but it is obviously false", cwd,
        dev);
